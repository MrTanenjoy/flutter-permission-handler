...
//  })
    ...
    private int checkPermissionStatus(@PermissionGroup int permission) {
        ...
        for (String name : names) {
            if (targetsMOrHigher) {
                final int permissionStatus = mContext.checkSelfPermission(name);
    ...
    private boolean shouldShowRequestPermissionRationale(int permission) {
        ...
        for (String name : names) {
            return mActivity.shouldShowRequestPermissionRationale(name);
        }
    ...
    private void requestPermissions(List<Integer> permissions) {
        ...
        if (permissionsToRequest.size() > 0) {
            mActivity.requestPermissions(requestPermissions, PERMISSION_CODE);
